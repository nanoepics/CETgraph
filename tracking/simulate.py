...
class SingleFrame:
    ...
    """
    def __init__(self, fov = [300, 200], numpar = None, difcon = 1, signal = 10, noise = 1, psize = 8, useRandomIntensity = True, numberPerSpecies = [None], signalPerSpecies = [None], pixelSizePerSpecies = [None], staticNoise = 0.0):
        ...
        for n in range(self.numpar):
            for x in range(psize):
                for y in range (psize):
                    r = 2*(x**2+4*y**2)/pixelSizePerParticle[n]**2
